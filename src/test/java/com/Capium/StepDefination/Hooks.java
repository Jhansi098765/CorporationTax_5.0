...
public class Hooks {
    ...
    @Before
    public void beforeScenario(Scenario scenario) {
//        HelperClass.setUpDriver(); // <== Init driver for each scenario
        scenarioTest = extent.createTest("Scenario: " + scenario.getName());
        System.out.println("Starting Scenario: " + scenario.getName());
        Log.info("Scenario: " + scenario.getName());
    }
    ...
    public void afterStep(Scenario scenario) {
        ...
        if (driver == null) {
            Log.error("Driver is null in afterStep. Skipping screenshot.");
            return;
        }
...
//                HelperClass.tearDown(); // Quit driver after scenario completes
                Log.info("Browser closed.");
            }
    ...
    // Screenshot utilities (unchanged)
    public static void captureScreenshotBase64(WebDriver driver, ExtentTest scenarioTest, String message) {
        try {
            TakesScreenshot ts = (TakesScreenshot) driver;
            String base64Screenshot = ts.getScreenshotAs(OutputType.BASE64);
